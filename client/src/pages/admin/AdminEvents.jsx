                ...
                {events.map(event => (
                    <div key={event._id} className="glass-panel event-card">
                        ...
                        <div className="event-content">
                            ...
                            <p className="subtitle" style={{ flex: 1, marginBottom: '1rem' }}>{event.description.substring(0, 100)}...</p>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', fontSize: '0.85rem', color: 'var(--text-light)', marginBottom: '1rem' }}>
                            ...
                            <button className="btn btn-danger" style={{ width: '100%', padding: '0.5rem' }} onClick={() => handleDelete(event._id)}>
                    ...
                    <div className="modal-content animate-fade-in">
                        ...
                        <form onSubmit={handleSubmit}>
                            <div className="form-group">
                                ...
                                <input type="text" required className="form-input" placeholder="e.g. Spring Open Day" value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} />
                            </div>
                            <div className="form-group">
                                <label className="form-label">Description</label>
                                <textarea required className="form-textarea" rows="3" placeholder="Describe what the event is about..." value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})}></textarea>
                            </div>
                            ...
                            <div className="form-group">
                                <label className="form-label">Event Image <span style={{ textTransform: 'none', fontWeight: 400, color: 'var(--text-light)', fontSize: '0.75rem' }}>(optional)</span></label>
                                <input type="file" accept="image/*" className="form-input" onChange={e => setFormData({...formData, image: e.target.files[0]})} />
